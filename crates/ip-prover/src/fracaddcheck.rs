...
/// `provers` pairs each remaining prover with its reduced fractional claim `(num, den)` at
/// `eval_point`.
pub struct BatchProveOutput<P: PackedField> {
	...
	/// Each remaining prover (with its final layer) paired with its reduced `(num, den)` fraction
	/// evaluation at `eval_point`.
	#[allow(clippy::type_complexity)]
	pub provers: Vec<((P::Scalar, P::Scalar), FracAddCheckProver<P>)>,
}
...
/// provers, each for an $m$-variate numerator/denominator pair, using
/// multilinear interpolation over `k` selector variables (where $n \le 2^k$). The combined claim is
...
/// its reduced fraction at the shared reduced `eval_point`) rather than combined into a single
/// claim, so the caller can finish the final layer itself.
///
/// The reduced proof is checked by the ordinary `binius_ip::fracaddcheck::verify` recursion over
/// `k + n_layers` variables, with the selector coordinates forming the first `k` coordinates of the
/// claim point — there is no separate batched verifier, mirroring prodcheck.
pub fn batch_prove<F: Field, P: PackedField<Scalar = F>>(
	...
	let (provers, claimed_fractions, eval_point) = (0..n_layers - 1).try_fold(
	...
	// Pair each remaining (single-layer) prover with its reduced fraction at `eval_point`.
	let provers = iter::zip(claimed_fractions, provers).collect();
...
fn batch_prove_layer<F: Field, P: PackedField<Scalar = F>>(
	...
	k: usize,
	channel: &mut impl IPProverChannel<F>,
) -> Result<(Vec<FracAddCheckProver<P>>, Vec<(F, F)>, Vec<F>), Error> {
	...
	// coordinates.
	let (mut layer_provers, next_provers): (Vec<_>, Vec<_>) =
		iter::zip(provers, &claimed_fractions)
			.map(|(prover, &(num, den))| {
				prover.layer_prover((
	...
	// Compute eq weights for batching over instances: eq(i, outer_coords) for all i in B_k.
	let eq_weights = eq_ind_partial_eval::<F>(outer_coords);

	// Each fracaddcheck layer batches its two (numerator, denominator) claims via a batching
	// coefficient. The verifier's `batch_verify_mle` samples one such coefficient per layer, before
	// the round polynomials, so we do the same and reuse it for both the content and selector
	// rounds.
	...
	for _round in 0..inner_coords.len() {
		let round_coeffs: RoundCoeffs<F> = iter::zip(&mut layer_provers, eq_weights.iter_scalars())
			.map(|(prover, eq_i)| Ok(combine_claims(prover.execute()?, batch_coeff) * eq_i))
	...
	let finished = layer_provers
		.into_iter()
		.map(|prover| {
			let evals: [F; 4] = prover
				.finish()?
				.try_into()
	...
	// selector buffers (not just the `n` real provers) keeps `claimed_fractions.len() == 2^k`, so
	// it stays aligned with the selector `eq` weights on subsequent layers; the padded entries are
	// zero.
	let next_claimed_fractions = izip!(&num_0s, &num_1s, &den_0s, &den_1s)
	...
	Ok((next_provers, next_claimed_fractions, next_point))
}
...
mod tests {
	...
	/// per-prover reduced fractions into a single [`FracAddEvalClaim`], matching the full-reduction
	...
	fn finish_batch_prove<F: Field, P: PackedField<Scalar = F>>(
		...
		let (claimed_fractions, provers): (Vec<_>, Vec<_>) = provers.into_iter().unzip();

		let (_provers, claimed_fractions, eval_point) =
			batch_prove_layer(provers, claimed_fractions, eval_point, k, channel).unwrap();

		let eq_weights = eq_ind_partial_eval::<F>(&eval_point[..k]);
		let num_eval =
			inner_product(claimed_fractions.iter().map(|&(n, _)| n), eq_weights.iter_scalars());
		let den_eval =
			inner_product(claimed_fractions.iter().map(|&(_, d)| d), eq_weights.iter_scalars());
	...
	fn test_batch_prove_verify_helper<P: PackedField>(n_layers: usize, n_provers: usize) {
		...
		// Run batch_prove (scalar fractions: empty content point), then finish the final layer.
		let mut prover_transcript = ProverTranscript::new(StdChallenger::default());
	...
	fn test_batch_prove_single_layer() {
		// n_layers=1 edge case: batch_prove runs 0 reductions and retains the single (final) layer,
		// which the test then finishes.
