...
///
/// [`SumcheckProver::finish`] returns the *padded* layer's child evaluations, which is what the
	...
	P: PackedField<Scalar = F>,
{
	...
	let inner_claims = [
		num_claim * pad_eq_inv,
		F::ONE + (den_claim - F::ONE) * pad_eq_inv,
	];
	let (inner, _cols) =
...
impl<F: Field, Inner: MleCheckProver<F>> SumcheckProver<F> for ZeroPadMleCheckProver<F, Inner> {
