...
impl<F: Field, P: PackedField<Scalar = F>> BivariateProductEvaluator<P> {
	/// Creates an evaluator for the claimed sum of the product of two columns of `store`.
	pub const fn new(store: &MleStore<'_, P>, cols: [ColId; 2], sum: F) -> Self {
		Self {
			cols,
			n_vars_remaining: store.n_vars(),
...
impl<F: Field, P: PackedField<Scalar = F>> RoundEvaluator<F, P> for BivariateProductEvaluator<P> {
	...
	fn fold(&mut self, challenge: F) {
		...
		// The store folds the columns; only the sum claim advances here.
