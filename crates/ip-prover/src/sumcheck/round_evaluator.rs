...
pub trait RoundEvaluator<F: Field, P: PackedField<Scalar = F>>: Send + Sync {
	...
	///
	/// See [`SumcheckProver::round_claim`] for the contract.
	...
	///
	/// `accum` is this evaluator's run of [`Self::degree`] slots, summed across all workers.
	...
	Evaluator: RoundEvaluator<F, P>,
{
	...
	fn round_claim(&self) -> Vec<F> {
		self.evaluators
			.iter()
			.map(|evaluator| evaluator.round_claim())
			.collect()
	}
	...
	fn execute(&mut self) -> Vec<RoundCoeffs<F>> {
		...
		iter::zip(&mut self.evaluators, offsets.windows(2))
			.map(|(evaluator, window)| evaluator.interpolate(&accum[window[0]..window[1]]))
			.collect()
	}
...
mod tests {
	...
	fn test_shared_final_layer_transcript_matches_old_provers() {
		for m in [1, 2, 3, 6] {
			...
			let num_evaluator = MleToSumCheckEvaluator::new(num_evaluator, z.to_vec());
			let den_evaluator = MleToSumCheckEvaluator::new(den_evaluator, z.to_vec());
			let product_0 = BivariateProductEvaluator::new(&store, [y_0_col, t_0_col], e_0);
			let product_1 = BivariateProductEvaluator::new(&store, [y_1_col, t_1_col], e_1);
