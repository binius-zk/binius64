	...
	for layer in packed_cutoff..layer_range.end {
		...
		for block in 0..1 << log_blocks_per_packed {
			...
			for j in 0..1 << log_half_block_size {
				packed_twiddle_offset = packed_twiddle_offset.set(block_start | j, twiddle0);
				packed_twiddle_offset = packed_twiddle_offset
					.set(block_start | j | (1 << log_half_block_size), twiddle1);
			}
		}
