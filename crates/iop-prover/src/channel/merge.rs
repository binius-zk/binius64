	...
	C: IOPProverChannel<P, A>,
{
	...
	fn flush(&mut self) {
		...
		for &k in &order {
			...
			place_block(&mut combined, self.pending[k].to_ref(), block_index);
			offset += 1 << n_k;
		}
		...
		let outer = self.inner.send_oracle(combined.to_ref());
...
{
	// Borrow exactly the destination's block at this index.
	//
	// The slice is the width of the source.
	// A block index of zero starts at the very first scalar.
	let n = src.log_len();
	let mut chunk = dst.chunk_mut(n, block_index);
	let mut chunk_buf = chunk.get();

	// Copy every scalar across.
	// Everywhere else in the destination stays as it was.
	for i in 0..1usize << n {
		chunk_buf.set(i, src.get(i));
	}
}
	...
	C: IOPProverChannel<P, A>,
{
	...
	fn send_oracle(&mut self, buffer: FieldSlice<P>) -> Self::Oracle {
		...
		self.pending
			.push(FieldBuffer::clone_from_slice(&self.alloc, buffer));
	...
	fn prove_oracle_relation(
		...
		place_block(&mut padded, transparent.to_ref(), block_index);
...
mod tests {
	...
	use binius_field::{
		BinaryField, BinaryField128bGhash, Field, PackedBinaryGhash1x128b, PackedBinaryGhash4x128b,
		PackedField,
	};
	...
	where
		P: PackedField<Scalar = BinaryField128bGhash>,
	{
		...
		for sizes in rounds {
			for _ in *sizes {
				...
				oracles.push(merge_prover.send_oracle(buffer.to_ref()));
				index += 1;
			}
	...
	fn multiple_relations_on_merged_oracle() {
		...
		let oracle_1 = merge_prover.send_oracle(buffer_1.to_ref());
		let oracle_2 = merge_prover.send_oracle(buffer_2.to_ref());
