...
/// Variable-length Keccak-256 gadget handle, built by [`keccak256_varlen`].
///
/// Bundles the input/output wires with the internal padded-message witness; fill the witness with
/// the `populate_*` methods.
pub struct Keccak256 {
	/// Prover-chosen message length in bytes (only `len_bytes <= capacity` is checked in-circuit).
	/// See the Soundness note on [`ByteVec`] for why callers must constrain it.
	pub len_bytes: Wire,
...
/// Keccak-256 of the first `message.len_bytes` bytes of a [`ByteVec`].
///
/// Constrains `digest == keccak256(message.data[..message.len_bytes])`; see [`ByteVec`] for the
/// little-endian byte encoding of `message.data`. Only `len_bytes <= capacity` is checked
/// in-circuit and the prover picks `len_bytes`, so callers must constrain it — see the Soundness
/// note on [`ByteVec`]. For a compile-time-constant length, [`fixed_length::keccak256`] is cheaper.
///
/// Returns a gadget handle; fill its witness with [`Keccak256::populate_len_bytes`],
/// [`Keccak256::populate_message`] and [`Keccak256::populate_digest`].
///
/// # Preconditions
/// * `message` has non-zero capacity.
pub fn keccak256_varlen(
	...
	let message = message.data.clone();
	...
	for block_no in 0..n_blocks {
		...
		for i in 0..N_WORDS_PER_BLOCK {
			xored_state[i] = b.bxor(state_in[i], padded_message[block_no * N_WORDS_PER_BLOCK + i]);
		}
	...
	let with_possible_end = b.bxor(expected_partial, b.select(is_not_last_column, zero, msb_one));
...
mod tests {
	...
	fn test_keccak_circuit(#[case] message_len_bytes: usize, #[case] max_message_len_bytes: usize) {
		...
		let byte_vec = ByteVec::new(message_wires, len);
		let keccak = keccak256_varlen(&b, &byte_vec, digest);
		let circuit = b.build();
	...
	// Characterization tests for the `len_bytes` caller obligation (see the Soundness note on
	// `ByteVec`). They drive the constraint system directly with all wires prover-populated,
	// modelling the composed-circuit case where `digest` is prover-influenced. The gadget
	// previously had no negative tests.
	...
	fn run_free_len(
		...
		let len = b.add_witness();
		let digest: [Wire; N_WORDS_PER_DIGEST] = std::array::from_fn(|_| b.add_inout());
		let data: Vec<Wire> = (0..cap_words).map(|_| b.add_inout()).collect();
		let byte_vec = ByteVec::new(data, len);
		let keccak = keccak256_varlen(&b, &byte_vec, digest);
		let circuit = b.build();
	...
	/// circuit is satisfiable. (Caller obligation, not a gadget bug — see the `ByteVec` Soundness
	/// note.)
	#[test]
	...
	fn test_constant_len_rejects_prefix_hash() {
		...
		let byte_vec = ByteVec::new_const_len(&b, data, msg.len());
		let keccak = keccak256_varlen(&b, &byte_vec, digest);
		let circuit = b.build();
