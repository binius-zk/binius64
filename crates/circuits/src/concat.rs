...
use binius_frontend::{CircuitBuilder, Wire, WitnessFiller, hints::ByteVecConcatHint};

use crate::{
	fixed_byte_vec::ByteVec,
	slice::{assert_slice_eq, create_byte_mask, extract_word, slice},
};
	...
	mod proptests {
		...
		proptest! {
			...
			fn correct_concatenation(terms in terms_strategy()) {
				...
				let datas: Vec<&[u8]> = terms.iter().map(|(d, _)| d.as_slice()).collect();
				let expected: Vec<u8> = datas.iter().flat_map(|d| d.iter().copied()).collect();
				let bytes = run_concat(&input_max_lens, &datas).unwrap();
